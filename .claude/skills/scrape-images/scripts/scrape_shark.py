#!/usr/bin/env python3
"""
Shark (sharkclean.com) 专用爬虫。
sharkninja CDN 的 JSON API 格式图片URL不能直接用 requests。
使用 Playwright 渲染页面后提取 <img> 实际 src。
"""
...
CATEGORIES = {
    "https://www.sharkclean.com/vacuum-cleaners/": ("吸尘器", "吸尘器"),
    "https://www.sharkclean.com/robot-vacuums/": ("吸尘器", "扫地机器人"),
    "https://www.sharkclean.com/steam-mops/": ("清洁机", "蒸汽拖把"),
    "https://www.sharkclean.com/air-purifiers/": ("空气净化器", "空气净化器"),
    "https://www.sharkclean.com/hair-care/": ("美发工具", "美发工具"),
}
    ...
    with sync_playwright() as p:
        ...
        for cat_url, (major, minor) in CATEGORIES.items():
            ...
            def handle_response(response):
                url = response.url
                if (('sharkninja.com' in url or 'cloudinary' in url)
            ...
            print(f'  拦截 {len(captured)} + DOM {len(dom_imgs)} 图片', flush=True)

            saved = 0
            # Process intercepted responses first
